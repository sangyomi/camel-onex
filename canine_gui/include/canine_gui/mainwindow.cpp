    ...
    , ui(new Ui::MainWindow)
{
    ...
    gpsTimer = new QTimer();
    connect(gpsTimer, SIGNAL(timeout()), this, SLOT(GridMapUpdate()));
    gpsTimer->start(1000);
...
void MainWindow::CameraInitialize()
{
...
void MainWindow::CameraUpdate()
{
    ...
    ui->CAMERA->setFixedSize(image.size());
}

void MainWindow::GridMapInitialize()
{
    ui->GRID_MAP->show();
}

void MainWindow::GridMapUpdate()
{
    GridM.getRandomCoordinate();
    GridM.GridMapReload();

    QImage image;

    if (image.load("/home/sangjun/camel-onex/canine_gui/image/gridmap.png"))
    {
        ui->GRID_MAP->setPixmap(QPixmap::fromImage(image));
        ui->GRID_MAP->setFixedSize(image.size());
    }
}
